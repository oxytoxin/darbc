...
<head>
    ...
    <title>RSBSA Layout Tuner — {{ $rsbsa->surname }}, {{ $rsbsa->first_name }}</title>
    <style>
        * { box-sizing: border-box; }
        ...
        .panel .row { display: flex; justify-content: space-between; margin-top: 6px; align-items: center; }
...
<body>
    ...
    <div class="help">
        ...
        (hold <b>Shift</b> for 5&nbsp;pt). For cell fields (DOB, PCN, mobile) adjust <b>gap</b> to tighten spacing.
        Click <b>Save positions</b>, then <b>Live PDF</b> to confirm. Fields shown as <code>〈name〉</code> have no value for
        this record but can still be positioned.
    </div>
    ...
    <div class="panel">
        ...
        <div class="hint">Values render at the same scale as the PDF — what you align here is what prints.</div>
    </div>

    <script>
        const S = {{ $S }};
        const FONT = {{ $fontSize }};
        const ASCENT = 0.8 * FONT;
        ...
        function applyInput() {
            ...
            if (!isNaN(x)) { state[k].x = x; selected.style.left = ptToLeft(x) + 'px'; }
