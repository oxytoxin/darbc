<x-forms::field-wrapper class="p-4 bg-white rounded shadow" :id="$getId()" :label="$getLabel()"
    ...
    ->toArray();
    @endphp
    <div {{ $attributes->merge($getExtraAttributes())->class(['filament-forms-repeater-component space-y-6 rounded-xl',
        'bg-gray-50 p-6' => $isInset(), 'dark:bg-gray-500/10' => $isInset() && config('forms.dark_mode')]) }}>
        @if (count($containers))
        <ul>
            <x-filament-support::grid :default="$getGridColumns('default')" :sm="$getGridColumns('sm')"
                :md="$getGridColumns('md')" :lg="$getGridColumns('lg')" :xl="$getGridColumns('xl')"
                :two-xl="$getGridColumns('2xl')" wire:sortable
                wire:end.stop="dispatchFormEvent('repeater::moveItems', '{{ $getStatePath() }}', $event.target.sortable.toArray())"
                class="gap-1">
                <ul class="flex gap-6 px-12 text-sm justify-items-stretch">
                    @foreach ($labels as $label)
                    <li class="flex-1 text-center whitespace-nowrap">{{ $label }}</li>
                    @endforeach
                </ul>
                ...
                <li x-data="{
                    ...
                    x-on:expand-concealing-component.window="
                                ...
                                " @class([ 'bg-white rounded-xl relative' , 'dark:bg-gray-800 dark:border-gray-600'=>
                    config('forms.dark_mode'),
                    ])>

                    <div class="flex gap-2" x-show="! isCollapsed">
                        @unless($isItemMovementDisabled)
                        <button title="{{ __('forms::components.repeater.buttons.move_item.label') }}" x-on:click.stop
                            ...
                            type="button"
                            @class([ 'flex items-center justify-center flex-none w-10 h-10 text-gray-400 border rounded transition hover:text-gray-500'
                        ...
                        <button title="{{ __('forms::components.repeater.buttons.delete_item.label') }}"
        ...
        <div class="relative flex justify-end">
            <x-forms::button :wire:click="'dispatchFormEvent(\'repeater::createItem\', \'' . $getStatePath() . '\')'"
                size="sm" type="button">
