<div>
    ...
    <div x-cloak x-data class="flex gap-2 mb-4 font-bold text-custom-blue">
        <div>
            <p>Release</p>
        ...
        <div>
            <p>From</p>
            <input wire:model="from" type="datetime-local" class="text-sm rounded">
        </div>
        <div>
            <p>To</p>
            <input wire:model="to" type="datetime-local" class="text-sm rounded">
        </div>
    </div>
    <div class="relative">
            ...
            <div class="flex flex-col justify-between p-3 mb-4 bg-white border rounded-md">
                ...
                <div class="leading-3">
                    <section class="flex items-center justify-between">
                        <h1 class="text-2xl font-medium text-custom-orange">{{ $selected_release->released_dividends_count }}</h1>
                        <h1 class="text-2xl font-bold text-custom-blue">{{ $selected_release->dividends_count }}</h1>
                    </section>
                    <progress id="file" value="{{ $selected_release->released_dividends_count }}" max="{{ $selected_release->dividends_count }}"></progress>
                    <section class="flex items-center justify-between mt-1">
                        <h1 class="text-sm font-medium text-gray-500">
                            {{ Akaunting\Money\Money::PHP(($selected_release->released_dividends_gross ?? 0) - ($selected_release->released_dividends_deductions ?? 0), false) }}</h1>
                ...
                <div class="mt-2 space-y-2 text-sm font-semibold uppercase divide-y-2">
                    <div class="flex items-center justify-between">
                        <p>Overall Number of Releases:</p>
                        <p class="text-xl text-custom-blue">{{ $selected_release->overall_released_dividends_count }}</p>
                    </div>
                    <div class="flex items-center justify-between">
                        <p>Overall Amount of Releases:</p>
                        <p class="text-xl text-custom-blue">{{ Akaunting\Money\Money::PHP(($selected_release->overall_released_dividends_gross ?? 0) - ($selected_release->overall_released_dividends_deductions ?? 0), false) }}</p>
                    </div>
                </div>
            </div>
        @endif
        <div wire:poll.2000ms class="grid grid-cols-3 gap-2 pb-4">
