    ...
    <tbody class="divide-y divide-gray-200">
        ...
        <tr>
            <td class="py-3 pl-3 text-sm font-bold bg-gray-100 border whitespace-nowrap">
                <a
                    href="{{ route('office-staff.manage-member-restrictions', ['member' => $user->member_information]) }}">
                    <p>{{ $user->full_name }}</p>
            ...
            @endphp
            @if ($dividend)
            <td class="whitespace-nowrap py-4 px-3 text-sm font-semibold text-center w-[7rem] border @switch($dividend->status)
            ...
            <td class="px-3 py-3 text-sm font-semibold text-center whitespace-nowrap">{{
                Akaunting\Money\Money::PHP($amount_to_claim ?? 0, true) }}</td>
        </tr>
        @empty
        @endforelse
    </tbody>
